...
rot = pin.SE3(pin.utils.rpyToMatrix(0, 90, 140), np.array([0, -0.05, 0.0]))
...
elif new_robot == "talos":
    rmodel, cmodel, vmodel = load_talos()
    print(rmodel)
...
target_poses = trajs[0]["object_trajectory_se3"]
se3_poses = list_to_se3(target_poses)
# print(se3_poses)
x0 = np.concatenate((pin.randomConfiguration(rmodel), np.zeros(rmodel.nv)))
