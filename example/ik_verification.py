...
def main():
    ...
    for axis, color in frame_colors.items():
        ...
        if axis == 'x':
            ...
            T_axis[:3, 3] = T_world_object[:3, 3] + T_world_object[:3, :3] @ np.array([frame_size/2, 0, 0])
        elif axis == 'y':
            # Rotate around z to point along y
            R_adjust = pin.utils.rpyToMatrix(-np.pi/2, 0, 0)
            T_axis[:3, :3] = T_world_object[:3, :3] @ R_adjust
            T_axis[:3, 3] = T_world_object[:3, 3] + T_world_object[:3, :3] @ np.array([0, frame_size/2, 0])
        elif axis == 'z':
            # Already pointing along z
            T_axis[:3, 3] = T_world_object[:3, 3] + T_world_object[:3, :3] @ np.array([0, 0, frame_size/2])
    ...
    for idx, (grasp_key, grasp) in enumerate(grasp_items):
        ...
        T_object_base = grasp_to_T(grasp, apply_tcp_offset=False, gripper_depth=gripper_depth)
        T_object_tcp = grasp_to_T(grasp, apply_tcp_offset=True, gripper_depth=gripper_depth)
        
        # Apply 90° Z-rotation to correct frame convention mismatch between GraspGen and robot TCP
        T_correction = np.eye(4)
        T_correction[:3, :3] = pin.utils.rpyToMatrix(0.0, 0.0, np.pi/2)  # 90° around Z
