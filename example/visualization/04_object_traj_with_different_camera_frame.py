"""Visualization of the offseted grasp trajectory in the robot frame (the transformation between the camera frame and the robot world is different to the identity).
...
if __name__ == "__main__":
    ...
    T_camera_world = pin.SE3.Identity()
    T_camera_world.translation = np.array([0, -1.0, -0.5])
    ...
    for k, pose_data in enumerate(object_trajectory_in_camera_frame):
        ...
        # Apply the full transformation chain including camera-to-world
        grasp_pose = (
            SE3_rot * T_camera_world * pose_data * best_grasp * offset_transform
        )
        scene[f"target_{k}"].pos[:] = grasp_pose.translation

        o.pose = (SE3_rot * T_camera_world * pose_data).homogeneous
