...
# Palette of distinct colors for multi-trajectory visualization
TRAJECTORY_COLORS = [
    [0.95, 0.26, 0.21],  # Red
    [0.13, 0.59, 0.95],  # Blue
...
class TrajectoryVisualizer:
    ...
    def __init__(
        ...
        # Track sphere names for cleanup
    ...
    def display_all_trajectories(
        ...
        Each trajectory gets a distinct color; the first waypoint is brighter.
        """
        if clear_previous:
        ...
        print("\nAll trajectories visible. Press Enter to continue...")
        input()
    ...
    def animate_joint_trajectory(
        ...
        Args:
            ...
            interactive: If True step-by-step (Enter / q); otherwise timed
            dt: Seconds between frames in non-interactive mode
        Returns:
        ...
        if interactive:
            print("\nControls: Enter = next frame,  q = quit")

        for k, q in enumerate(joint_trajectory):
            if interactive:
                user_input = input(
                    f"  Frame {k}/{len(joint_trajectory) - 1} (Enter / q): "
                )
    ...
    def animate_ocp_solution(
        ...
        Args:
            ...
            interactive: Step-by-step (Enter / q) or timed
            dt: Seconds between frames in non-interactive mode
        """
        if interactive:
            print("\nControls: Enter = next frame,  q = quit")
        ...
        for k, xs in enumerate(ocp_states):
            if interactive:
                user_input = input(f"  Frame {k}/{len(ocp_states) - 1} (Enter / q): ")
                if user_input.strip().lower() == "q":
                    break
            else:
                time.sleep(dt)
    ...
    # ------------------------------------------------------------------
    # Replay all saved trajectories (robot moves!)
    # ------------------------------------------------------------------
    ...
    def replay_all_saved_trajectories(
        ...
        For each trajectory:
          1. (Optional) Show target waypoints as colored spheres
          2. Move the robot through the IK or OCP joint trajectory
          3. Move the object in sync
          4. Clean up spheres before the next trajectory

        The TCP trajectory is always recomputed on the fly from the saved
        camera_translation so that grid-search files (which don't store TCP
        poses) work just as well as kept-trajectory files.

        Args:
            loader: TrajectoryLoader instance
            rmodel / rdata: Pinocchio model / data
            robot_config: RobotConfig (contains gripper_depth, dt …)
            dataloader: DataLoader (for computing TCP from camera_translation)
            solution_type: 'ik' or 'ocp' ('ocp' falls back to 'ik' if unavailable)
            interactive: True = Enter to advance; False = timed playback
            dt: Seconds between frames (non-interactive mode)
        ...
        print(
            f"Mode          : {'interactive' if interactive else f'timed (dt={dt}s)'}"
        )
        print(f"{'=' * 60}")
        ...
        for idx in range(num):
            ...
            # ----------------------------------------------------------
            # Compute TCP trajectory on the fly (works for all file types)
            # ----------------------------------------------------------
            ...
            if interactive:
                input(
                    f"  Press Enter to start animation (trajectory {idx + 1}/{num})..."
                )
            ...
            # ----------------------------------------------------------
            # Clean up spheres before next trajectory
            # ----------------------------------------------------------
            if show_waypoints:
                self.clear_waypoints(prefix)
