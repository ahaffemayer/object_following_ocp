...
if __name__ == "__main__":
    ...
    config_path = pathlib.Path(
        "/workspaces/object_following_ocp/example/robot_motion/configs/ocp_config_kinova.yml"
    )
    ...
    ee_traj_world *= pin.SE3(pin.exp3(np.array([0, 0, np.deg2rad(90)])), np.zeros(3))
    ...
    tcp_traj_world *= pin.SE3(pin.exp3(np.array([0, 0, np.deg2rad(90)])), np.zeros(3))
